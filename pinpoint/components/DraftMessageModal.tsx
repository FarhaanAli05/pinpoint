...
export function DraftMessageModal({ pin, onClose }: DraftMessageModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-xl border border-border bg-surface shadow-2xl shadow-black/40">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <h2 className="text-base font-bold text-foreground">Draft Message</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-5">
          ...
          {loading ? (
            ...
            <textarea
              ...
              rows={10}
              className="w-full resize-none rounded-lg border border-border bg-card p-3 text-sm leading-relaxed text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
            />
          )}
        </div>
        ...
        <div className="flex gap-2 border-t border-border px-5 py-4">
          ...
          <button
            ...
            disabled={loading}
            className="rounded-lg border border-border px-5 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-surface-elevated disabled:opacity-50"
          >
            Mark as Sent
          </button>
        </div>
